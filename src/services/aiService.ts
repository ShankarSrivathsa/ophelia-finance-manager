...
class AIService {
  ...
  private initializeConfig() {
    ...
    if (geminiKey && geminiKey !== 'your_gemini_api_key_here') {
      ...
      console.warn('⚠️ No valid AI provider configured. Please add a valid API key to your .env file.');
    }
  }
  ...
  private async callGeminiAPI(prompt: string): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // If the model is not found, try the legacy model
        if (response.status === 404) {
          return await this.callGeminiAPILegacy(prompt);
        }
  ...
  private async callGeminiAPILegacy(prompt: string): Promise<string> {
    if (!this.config || this.config.provider !== 'gemini') {
      throw new Error('Gemini API not configured');
    }
    ...
    try {
      ...
      console.error('Gemini Legacy API call failed:', error);
      throw error;
    }
  }
  ...
  private async callAI(prompt: string): Promise<string> {
    if (!this.config) {
      throw new Error('No AI provider configured. Please add a valid API key for Gemini, OpenAI, or Anthropic to your .env file.');
    }
...
${data.expenses.slice(0, 10).map(exp => `- ${exp.category}: ${formatCurrency(exp.amount)} (${exp.description})`).join('\n')}

BUDGETS:
${data.budgets.map(budget => `- ${budget.category}: ${formatCurrency(budget.spent)}/${formatCurrency(budget.amount)} (${((budget.spent/budget.amount)*100).toFixed(1)}% used)`).join('\n')}

SAVINGS GOALS:
${data.savingsGoals.map(goal => `- ${goal.name}: ${formatCurrency(goal.currentAmount)}/${formatCurrency(goal.targetAmount)} (${((goal.currentAmount/goal.targetAmount)*100).toFixed(1)}% complete, target: ${goal.targetDate})`).join('\n')}
...
${data.budgets.map(budget => {
  ...
  return `- ${budget.category}: ${formatCurrency(budget.spent)}/${formatCurrency(budget.amount)} (${percentage.toFixed(1)}% - ${status})`;
}).join('\n')}
...
${Object.entries(data.expenses.reduce((acc, exp) => {
  ...
  return acc;
}, {} as Record<string, number>)).map(([cat, amount]) => `- ${cat}: ${formatCurrency(amount)}`).join('\n')}

FINANCIAL GOALS:
${data.savingsGoals.map(goal => `- ${goal.name}: Need ${formatCurrency(goal.targetAmount - goal.currentAmount)} more`).join('\n')}
...
`;
      ...
      if (budget.amount > 0) {
        ...
        if (percentage > 100) {
          recommendations.push({
            ...
            action: `Reduce ${budget.category} spending by ${formatCurrency(budget.spent - budget.amount)}`,
  ...
  private getFallbackBudgetOptimization(data: FinancialData): BudgetOptimizationResponse {
    const adjustments = data.budgets
      .filter(budget => budget.amount > 0 && (budget.spent / budget.amount) > 1.1) // Over budget by 10%
      .map(budget => ({
        category: budget.category,
  ...
  private getFallbackReport(data: FinancialData, reportType: string): ReportResponse {
    ...
    return {
      ...
      summary: `This period you earned ${formatCurrency(totalIncome)} and spent ${formatCurrency(totalExpenses)}, resulting in a net ${netIncome >= 0 ? 'income' : 'loss'} of ${formatCurrency(Math.abs(netIncome))}.`,
      sections: [
        {
          title: 'Income Analysis',
          content: `Your total income for this period was ${formatCurrency(totalIncome)}. This represents your earning capacity during the analyzed timeframe.`
        },
        {
          title: 'Expense Analysis', 
          content: `Your total expenses were ${formatCurrency(totalExpenses)}. Review your spending categories to identify optimization opportunities.`
        }
      ],
      keyMetrics: [
        {
